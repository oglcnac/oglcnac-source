(function () {
  ...
  function renderSiteMap(rows) {
    ...
    byId("workbench-site-map").innerHTML = [...groups].map(([id, sites]) => `<div class="site-track"><strong>${escapeHtml(id)}</strong><div class="site-track-line">${sites.map((site) => `<span style="left:${Math.max(0, Math.min(100, site.position / site.sequence_length * 100))}%" title="${site.residue}${site.position}: ${site.probability.toFixed(3)}"></span>`).join("")}</div><small>${sites.length} S/T candidates across ${sites[0].sequence_length} residues</small></div>`).join("");
  }
  ...
  function renderRows() {
    ...
    byId("workbench-table").querySelector("tbody").innerHTML = rows.map((row) => `<tr><td>${escapeHtml(row.protein_id)}</td><td>${row.species}</td><td>${row.sequence_length}</td><td>${row.residue}${row.position}</td><td><code>${escapeHtml(row.sequence_window)}</code></td><td>${row.probability.toFixed(3)}</td><td>${escapeHtml(row.confidence_band)}</td><td>${escapeHtml(row.model_version)}</td><td>${escapeHtml(row.atlas_status)}</td><td>${row.atlas_record_count}</td><td>${escapeHtml(row.atlas_pmids.join("; "))}</td><td>${escapeHtml(row.ogt_pin_status)}</td><td>${row.ogt_pin_evidence_count}</td></tr>`).join("");
  }

  async function finish(predictions) {
    const [atlas, ogtPin, manifest] = await Promise.all([window.OglcnacStaticData.loadAtlasRecords(), window.OglcnacStaticData.loadOgtPinRecords(), fetch("/static/prediction/v1/manifest.json").then((response) => response.json())]);
    allRows = window.OglcnacWorkbenchCore.enrichPredictions({ records: activeRecords, predictions, indexes: window.OglcnacWorkbenchCore.buildEvidenceIndexes(atlas, ogtPin), species: activeSpecies, modelVersion: manifest.version });
  ...
  function ensureWorker() {
    ...
    worker.addEventListener("message", (event) => {
      ...
      if (message.type === "result") finish(message.results || []).catch((error) => fail(error.message));
  ...
  document.addEventListener("DOMContentLoaded", () => {
    byId("workbench-sample").addEventListener("click", () => { byId("workbench-species").value = "human"; byId("workbench-fasta").value = ">sp|P18583|SON_HUMAN sample\nAAAAAAAAAAAAAASAAAAAAAAAAAAAATAA"; });
    ...
    byId("workbench-form").addEventListener("submit", (event) => {
      ...
      try { activeRecords = window.OglcnacPredictionCore.validateFasta(fasta, activeSpecies); if (!window.OglcnacPredictionCore.createCandidates(activeRecords).length) throw new Error("No S/T residues were found in the FASTA input."); } catch (error) { fail(error.message); return; }
