...
def AAindex_hm_encoding(file_name):
    AAdirname = os.path.dirname(__file__)
    with open(os.path.join(AAdirname,"AAindex_normalized.txt")) as f:
...
def AAindex_ms_encoding(file_name):
    # C:\Users\jc\Documents\GitHub\O-GlcNAcDB\O_GlcNac_DB\AAindex\AAindex_normalized.txt
    AAdirname = os.path.dirname(__file__)
    with open(os.path.join(AAdirname,"AAindex_normalized.txt")) as f:
        records = f.readlines()[1:]
    AA_aaindex = 'ARNDCQEGHILKMFPSTWYVX'
